...
async function toggleFavorito(filme, id_usuario) {
    ...
    try {
        if (btnFavorito.classList.contains('favoritado')) {
            ...
            const tipo = (filme.Type || '').toLowerCase() === 'series' ? 'series' : 'filmes';
            setTimeout(() => {
                window.location.href = 'acesso.html?tab=' + tipo;
            }, 800);
        }
    } catch (error) {
...
async function carregarDetalhes() {
    ...
    try {
        ...
        if (filme && typeof filme.body !== 'undefined') {
            filme = typeof filme.body === 'string' ? JSON.parse(filme.body) : filme.body;
        }
