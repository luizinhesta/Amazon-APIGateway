...
class BancoDados {
    ...
    async buscarUsuarioPorLogin(login) {
        try {
            ...
            const tentativas = [
                { method: "GET", url: apiUrl(`usuario?login=${encodeURIComponent(login)}`) },
                { method: "GET", url: apiUrl(`editar?login=${encodeURIComponent(login)}`) }
            ];

            for (const t of tentativas) {
                try {
                    const response = await fetch(t.url, { method: t.method });
                    const data = await this.tratarResposta(response);
            ...
            console.error("? Erro buscar usuário por login:", error);
            return null;
        }
    }

    async editarUsuario(dadosEdicao) {
        try {
            const payload = { ...dadosEdicao };

            const tentativas = [
                { method: "PUT", url: apiUrl("editar") },
                { method: "POST", url: apiUrl("editar") },
                { method: "PUT", url: apiUrl("usuario") },
                { method: "PATCH", url: apiUrl("usuario") }
            ];
            ...
            for (const t of tentativas) {
                try {
                    ...
                    if (data?.sucesso === false) {
                        throw new Error(data?.erro || data?.message || "Erro ao editar usuário");
                    }
            ...
            if (ultimoErro && /Failed to fetch|404|NetworkError/i.test(String(ultimoErro.message || ultimoErro))) {
                throw new Error("A API atual não possui endpoint de edição de usuário publicado.");
            }

            throw ultimoErro || new Error("Não foi possível atualizar os dados");
        } catch (error) {
    ...
    async buscarFilmes({ tipo = "movie", query = "", ano = "" } = {}) {
        try {
            ...
            filmes = filmes.map(f => ({
                ...
                Type: f.Type || f.tipo || "",
                Plot: f.Plot || f.plot || f.resumo || f.sinopse || f.overview || f.Description || ""
            }));
